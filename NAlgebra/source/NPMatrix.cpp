...
NPMatrix::NPMatrix(const NVector &u, ul_t n, ul_t p) :
...
NVector NPMatrix::row(ul_t i) const {
...
NVector NPMatrix::col(ul_t j) const {
...
vector<NVector> NPMatrix::rows(ul_t i1, ul_t i2) const {
...
vector<NVector> NPMatrix::cols(ul_t j1, ul_t j2) const {
...
void NPMatrix::setRow(const NVector &u, ul_t i1) {
    assert(u.dim() <= _p && isValidRowIndex(i1));

    std::copy(u.begin(), u.end(), this->begin() + _p * i1);
}

void NPMatrix::setCol(const NVector &u, ul_t j1) {
...
void NPMatrix::setRows(const vector<NVector> &vectors, ul_t i1) {
...
void NPMatrix::setCols(const vector<NVector> &vectors, ul_t j1) {
